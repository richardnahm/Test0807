...
- [ODBC User\'s Manual](#odbc-user%5Cs-manual)
  ...
  - [1. Introduction](#1%EA%B0%9C%EC%9A%94)
    ...
    - [ODBC API](#odbc-api)
  - [2. Installing and configuring the Altibase ODBC](#2%EC%84%A4%EC%B9%98-%EB%B0%8F-%EC%84%A4%EC%A0%95)
...
Homepage: http://www.altibase.comhttp://www.altibase.com/)
...
-   Experience in a database server, operating system, or network administration
...
This section describes the conventions used in this manual. Understanding these conventions will make it more convenient to find information in this manual and other manuals in the series.

| Rule      | Meaning                                                      |
| --------- | ------------------------------------------------------------ |
| Italic    | Indicates book title, emphasis or placeholder variables for which particular values are supplied by the user |
...
When you need immediate assistance regarding technical issues, please contact Altibase's Customer Support site (http://altibase.com/support-center/).

Thank you. We always appreciate your feedback and suggestion.
...
ODBC (Open Database Connectivity) is a standard programming language interface for accessing databases, which is developed by Microsoft. In order to access an ODBC Application database(written using the ODBC AAPI), ODBC software (e.g ODBC Driver Manager) and an additional module or driver are required. Altibase ODBC driver conforms to ODBC 3.51 specifications for this purpose.
...
For further information on the SQL data types supported by Altibase and their mapping relations with ODBC data types, please refer to "B, Appendix: Data Types" in the Altibase CLI User's Manual.
...
Based on the conformation of ODBC functions, this section explains three functions: currently supported by Altibase ODBC, those that are soon to be supported, and those that are not supported.
...
| Function Name       | Level  | Support Status | Future Support | Remarks                               |
| ------------------- | ------ | -------------- | -------------- | ------------------------------------- |
...
2. Installing the Altibase ODBC Driver
------------
...
For Unix-like operating system, the Altibase ODBC driver is installed when the Altibase server or client package is installed. For information on how to install the Altibase server or client package, please refer to the Installation Guide.
...
The reason why two drivers are included in the 64-bit package is as follows. The size of the SQLLEN type is defined by 64-bit ODBC Driver Managers as 64 bits. However, as unixODBC (one of the ODBC Driver Managers available for use in the Unix-like operating systems) defines the size of the SQLLEN type to 32 or 64 bites, depending on the version and compile option, Altibase provides two drivers to offer a wide range of support. When using unixODBC, it is recommeded that you select the driver that matches the SQLLEN size.
...
In order to use the ODBC driver in Unix, you must first install the ODBC Driver Manager. The unixODBC Driver Manager and iODBC Driver Manager are ODBC driver managers that are available for use in Unix. For more detailed information on each driver manager, please refer to the following links: 
...
-   host (name or IP): Enter the hostname or IP address of the computer on which Altibse server to be accessed is running.
...
\You can check whether the ODBC driver is successfully connected to the database by clicking "Test Connection". Once you click "OK", you will be able to see the data source added to the DSN tab in the name you have just entered.

3. ODBC Programming
---------------

This chapter describes how to write an ODBC application using the Altibase ODBC Driver with examples.
...
Below is an example code of an ODBC application connecting to the Altibase server and its execution results.
...
If you run the generated exe file after compiling in Visual C++, you can see the following result:
...
The LongDataCompat attribute must also be set to ON in the connection string as shown below: 
